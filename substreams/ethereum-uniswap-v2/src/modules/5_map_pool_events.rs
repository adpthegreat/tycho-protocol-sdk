...
use crate::abi::{self};
use substreams::prelude::{StoreGetBigInt, BigInt};
...
pub fn map_pool_events(
    ...
    // assign them 
    //since there are multiple txs in a block we have to emit the right balance changes per tx
    //with the reserve amount we will do this using the ordinal 
    ...
    let mut balances_map: HashMap<String, Vec<(u64, BigInt)>> = HashMap::new();
    ...
    // Build cumulative balances in balances_map
    ...
    handle_transfers(&block, &mut tx_changes, &pools_store, &mut balances_map);
...
fn handle_sync(
    ...
    let mut on_sync = |event: Sync, _tx: &eth::TransactionTrace, _log: &eth::Log| {
        ...
        // Update balance changes for each token (create balance changes for only token_a and token_b)
...
fn handle_transfers(
    ...
    tx_changes: &mut HashMap<Vec<u8>, PartialChanges>,
    store: &StoreGetProto<ProtocolComponent>,
    balances_map: &mut HashMap<String, Vec<(u64, BigInt)>>,
) {
    let mut on_transfer = |event: Transfer, tx: &eth::TransactionTrace, log: &eth::Log| {
        ...
        if let Some(pool) = pool {
            ...
            let get_balance_at_ord = |key: &str, ord: u64| -> Option<BigInt> {
                balances_map.get(key).and_then(|entries| {
...
fn merge_block(
    ...
    let trace_data = TraceData::Rpc(RpcTraceData {
        caller: None,
        calldata: abi::pool::functions::TotalSupply {}.encode()
    });

    // let mut entrypoints = HashSet::new();
    // let mut entrypoint_params = HashSet::new();

    //create entrypoints for all components 
    // Add created pools to the tx_changes_map
    for mut change in block_entity_changes
        .changes
        .clone()
        .into_iter()
    {
        // Create entrypoints for each component before inserting into map
        for component in change.component_changes.iter() {
            ...
            change.entrypoints.push(entrypoint);
            change.entrypoint_params.push(entrypoint_params);
        }
        ...
        tx_entity_changes_map
            ...
            .and_modify(|c: &mut TransactionChanges| {
                ...
                c.entrypoints
                    .extend(change.entrypoints.clone());
                c.entrypoint_params
                    .extend(change.entrypoint_params.clone());
            })
            .or_insert(change);
    }
